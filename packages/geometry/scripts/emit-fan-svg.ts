...
import {
  deriveFrustum, buildFanOutline, buildOverlapStrip, fanBounds,
...
const overlap = buildOverlapStrip(profile, g, 128);
...
<svg xmlns="http://www.w3.org/2000/svg" width="${W.toFixed(3)}mm" height="${H.toFixed(3)}mm"
  ...
  <g transform="translate(${tx.toFixed(4)} ${ty.toFixed(4)})">
    <path d="${outlineToSvgPath(bleed)}" fill="none" stroke="#e08" stroke-width="0.25" stroke-dasharray="2 1"/>
    ...
    <polygon points="${poly(overlap)}" fill="#0a04" stroke="#0a0" stroke-width="0.2"/>
  </g>
...
console.log(`  blank bbox  ${b.widthMm.toFixed(2)} x ${b.heightMm.toFixed(2)} mm (incl. bleed)`);
